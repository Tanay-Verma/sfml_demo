...
int main(int argc, char *argv[])
{
    ...
    sf::RenderWindow window(sf::VideoMode(wWidth, wHeight), "Wohoo...");
    ...
    // scale the imgui ui and test size by 2
    ...
    // text character size is in pxels, so move the text up from the bottom by it's height
    ...
    while (window.isOpen())
    {
        ...
        // update the imgui for this fram with the time that the last frame took
        ...
        if (drawText) // draw the test if the boolean is true
        {
            window.draw(text);
        }
        ImGui::SFML::Render(window); // draw the ui so it's on top
